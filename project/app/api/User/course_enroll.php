...
header('Location:course_enroll.php?uname=$_SESSION["username"]&register=empty');
exit();
}
     $sql="INSERT INTO enroll(s_id,c_id,c_name) values($id,$c_id,'".$c_name."')";
     if(mysqli_query($conn,$sql)){
        header('Location:course_enroll.php?uname=$_SESSION["username"]&register=success');
      exit();
     }
}
}
...
<h1><span class="yellow">Course Enrollment Form</span></h1>
<div class="box">
  
<form action="/app/api/user/course_enroll.php" method="POST">

<div>
      <input type="text" name="c_name"  placeholder="CourseName:">
 </div>

    <div>
      <input type="text" name="c_id"  placeholder="Course_ID:">
    </div>
  ...
  <?php
                $fullurl="http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";  
                 ...
                 if(strpos($fullurl,"register=empty")==true){
                  echo "<html>
	<body>
	<p  style='color:white;' class='error' > You did not fill in all the fields!</p>
	</body>
	</html>";
	exit();
                 }
                 elseif(strpos($fullurl,"register=success")==true){
                  echo "<html>
	<body>
	<p style='color:white;' class='error' > Registration successful!</p>
	</body>
	</html>";
	exit();
                 }
