...
exit();
    ...
    $conn = new mysqli('localhost', 'root', 'roopav@123', 'project');
      ...
      $email = $conn->real_escape_string($_POST['email']);
        ...
        $sql = $conn->query("SELECT id FROM student WHERE email='$email'");
    ...
    $conn->query("UPDATE student SET token='$token', tokenExpire=DATE_ADD(NOW(), INTERVAL 5 MINUTE)
   WHERE email='$email'");
...
<br> <input class="form-control" id="email" placeholder="Email Address"><br>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function () {
     ...
     $('.btn-primary').on('click', function () {
       ...
       $.ajax({
  url: 'forgotPassword.php', method: 'POST', dataType: 'json', data: {
